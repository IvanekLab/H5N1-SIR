# Main model - Based on Slide #2 in Slide Deck (v7a)
...
HPAI_dyn <- function(time, state, parameters) {
  with(as.list(c(state, parameters)), {
    ...
    dI_Lc <- f * I_Dc + p * lambda_L * S_L - (d + mu1 + mu2 + alpha_c + ro * sigma + delta_1 + delta_2) * I_Lc
    dI_Ls <- f * I_Ds + (1 - p) * lambda_L * S_L - (d + mu1 + mu2 + alpha_s) * I_Ls
    dH_L <- f * H_D + ro * sigma * I_Lc - (d + mu1 + mu2 + alpha_H + delta_1 + delta_2) * H_L
    dR_L <- f * R_D + alpha_s * I_Ls + alpha_c * I_Lc + alpha_H * H_L - (d + mu1 + mu2 + gamma) * R_L
    dR_Lc <- f * R_Dc + alpha_c * I_Lc + alpha_H * H_L - (d + mu1 + mu2 + gamma) * R_Lc
    dR_Ls <- f * R_Ds + alpha_s * I_Ls - (d + mu1 + mu2 + gamma) * R_Ls
    
    dS_D <- gamma * R_D + d * S_L - lambda_D * S_D - (f + mu1 + mu2) * S_D
    dI_Dc <- d * I_Lc + p * lambda_D * S_D - (f + mu1 + mu2 + alpha_c + ro * sigma + delta_1 + delta_2) * I_Dc
    dI_Ds <- d * I_Ls + (1 - p) * lambda_D * S_D - (f + mu1 + mu2 + alpha_s) * I_Ds
    dH_D <- d * H_L + ro * sigma * I_Dc - (f + mu1 + mu2 + alpha_H + delta_1 + delta_2) * H_D
    dR_D <- d * R_L + alpha_s * I_Ds + alpha_c * I_Dc + alpha_H * H_D - (f + mu1 + mu2 + gamma) * R_D
    dR_Dc <- d * R_Lc + alpha_c * I_Dc + alpha_H * H_D - (f + mu1 + mu2 + gamma) * R_Dc
